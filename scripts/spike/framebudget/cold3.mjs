...
const DIR = path.join(path.dirname(fileURLToPath(import.meta.url)), 'dist3');
const MIME={'.html':'text/html','.js':'text/javascript','.css':'text/css'};
const srv=http.createServer((q,s)=>{const f=path.join(DIR,q.url==='/'?'index3.html':q.url.split('?')[0]);
...
for (const cpu of [1,6]) for (let i=0;i<3;i++) {
  ...
  await pg.goto('http://127.0.0.1:8805/',{waitUntil:'load'});
