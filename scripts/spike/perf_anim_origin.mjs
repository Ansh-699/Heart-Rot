...
const PORT=9337;
const c=spawn('/usr/bin/google-chrome',['--headless=new',`--remote-debugging-port=${PORT}`,`--user-data-dir=${DIR}/prof4`,'--no-first-run','about:blank'],{stdio:'ignore'});
let v; for(let i=0;i<60;i++){try{v=await(await fetch(`http://127.0.0.1:${PORT}/json/version`)).json();break}catch{await sleep(250)}}
const t=await(await fetch(`http://127.0.0.1:${PORT}/json/new?file:///home/anshtyagi/Documents/pixel-artgame/scripts/spike/perf_anim_origin.html`,{method:'PUT'})).json();
