for (const pick of wanted) {
  ...
  try {
    const res = await matchSettle(env, { privyToken: 'probe', arenaId: pick.id.toString() }, ctx);
...
/**
 * Replay the Worker's own `matchSettle` against the newest settled WIN arena, as the seat
 * that fought it (identity read off the roster, fed to the auth stub). Prints every step
 * and any throw in full. `pnpm`-free: bundle with esbuild via scripts/ops/workerprobe.sh's
 * recipe, then `REPO=$PWD node <bundle>`.
 */
...
const t0 = performance.now(); const at = () => `${((performance.now() - t0) / 1000).toFixed(1).padStart(5)}s`;
...
for (let step = 0; step < 40; step++) {
  ...
  const d = await acct(rpc as never, pdas.arena); if (!d) continue;
  let a; try { a = decodeArena(d); } catch { console.log(`${at()}  ${id} undecodable`); continue; }
  const pd = await acct(rpc as never, pdas.players); const roster = pd ? decodePlayers(pd) : null;
  const seated = roster?.slots.filter((s) => s.occupied) ?? [];
  ...
  if (a.phase === 3 && a.outcome === 1 && seated.length > 0) {
    picks.push({ id, identity: seated[0]!.identity });
  }
}
if (picks.length === 0) { console.log('no settled WIN arena with a seated raider in range'); process.exit(1); }
const only = process.env.ONLY_ARENAS ? new Set(process.env.ONLY_ARENAS.split(',')) : null;
const wanted = only ? picks.filter((p) => only.has(p.id.toString())) : picks.slice(-1);
for (const pick of wanted) {
  console.log(`${at()}  replaying matchSettle on ${pick.id} as identity ${Buffer.from(pick.identity).toString('hex').slice(0, 16)}…`);
  process.env.PROBE_IDENTITY_HEX = Buffer.from(pick.identity).toString('hex');
  try {
...
const only = process.env.ONLY_ARENAS ? new Set(process.env.ONLY_ARENAS.split(',')) : null;
const wanted = only ? picks.filter((p) => only.has(p.id.toString())) : picks.slice(-1);
const devVars = Object.fromEntries(readFileSync(`${process.env.REPO}/worker/.dev.vars`, 'utf8').split('\n').filter((l) => l.includes('=')).map((l) => [l.slice(0, l.indexOf('=')).trim(), l.slice(l.indexOf('=') + 1).trim().replace(/^"|"$/g, '')]));
const treasuryJson = JSON.parse(readFileSync(`${homedir()}/.config/heartrot/treasury.json`, 'utf8')) as number[];
const env = { ...devVars, TREASURY_SECRET_KEY: getBase58Decoder().decode(Uint8Array.from(treasuryJson)), PROGRAM_ID: 'JCfWB9zDXYqAv2or2GVriN39enEudWstz3M8sKvVkzc5', ROUTER_ENDPOINT: ROUTER, VALIDATOR_IDENTITY: 'MAS1Dt9qreoRMQ14YQuhg8UTZMMzDdKhmkZMECCzk57', BASE_RPC_URL: devVars.BASE_RPC_URL ?? BASE } as never;
const background: Promise<unknown>[] = [];
const ctx = { waitUntil: (p: Promise<unknown>) => void background.push(p.catch((e) => console.error('background:', e))) };
try {
  const res = await matchSettle(env, { privyToken: 'probe', arenaId: pick.id.toString() }, ctx);
  console.log(`${at()}  matchSettle -> ${res.status} ${(await res.text()).slice(0, 400)}`);
} catch (error) {
  console.log(`${at()}  matchSettle THREW: ${error instanceof Error ? `${error.name}: ${error.message}` : String(error)}`);
  const cause = (error as { cause?: unknown }).cause;
  if (cause !== undefined) console.log('cause:', cause instanceof Error ? `${cause.name}: ${cause.message}` : JSON.stringify(cause).slice(0, 600));
